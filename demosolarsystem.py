...
class Planet:
    ...
    def attraction(self, other):
        #Working out the gravitational force of attraction between two planets
        other_x, other_y = other.x, other.y
        distance_x = other_x - self.x
        distance_y = other_y - self.y
        distance = math.sqrt(distance_x**2 + distance_y**2) #Distance between 2 objects using pythagoras theorem

        if other.sun:
            self.distance_to_sun = distance
        
        force = self.G*self.mass*other.mass/(distance**2)  #F = GMm/r^2
        theta = math.atan2(distance_y, distance_x)  #The angle of the objects
    ...
    def update_position(self, planets):
        ...
        self.x_val += (total_fx/self.mass)*self.TIMESTEP  #a = F/m
        self.y_val += (total_fy/self.mass)*self.TIMESTEP

        self.x += self.x_val * self.TIMESTEP  #v = at or acceleration = velocity/time
